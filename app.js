...
function resellerOrderRow(order) { return `<div class="order-row"><div><strong>${order.product}</strong><small>${order.id} · ${order.createdAt}</small></div><div><strong>${money(order.sellingPrice)}</strong><small class="profit">+${money(order.profit)} profit</small></div><div><span class="status ${order.status.toLowerCase()}">${order.status}</span></div><div>${order.tracking ? `<a href="${order.tracking}" target="_blank">View tracking →</a>` : `<small>Tracking appears after shipping.</small>`}</div></div>`; }
function wholesalerOrderRow(order) { return `<div class="order-row"><div><strong>${order.product}</strong><small>${order.id} · buyer: ${order.buyer}</small></div><div><strong>${money(order.cost)}</strong><small>Wholesale value</small></div><div><select data-status-for="${order.id}"><option ${order.status === "Pending" ? "selected" : ""}>Pending</option><option ${order.status === "Shipped" ? "selected" : ""}>Shipped</option><option ${order.status === "Delivered" ? "selected" : ""}>Delivered</option></select></div><div><input data-tracking-for="${order.id}" placeholder="Tracking URL" value="${order.tracking || ""}" /><button data-save-order="${order.id}">Save</button></div></div>`; }
function updateOrder(orderId) { const order = state.orders.find(item => item.id === orderId); const status = document.querySelector(`[data-status-for="${orderId}"]`).value; const tracking = document.querySelector(`[data-tracking-for="${orderId}"]`).value.trim(); if (status === "Delivered" && !tracking) return showToast("Add a tracking link before marking delivered."); order.status = status; order.tracking = tracking; if (status === "Delivered" && !order.commissionCredited) { addWallet(order.resellerEmail, order.profit); order.commissionCredited = true; showToast(`Delivered. ${money(order.profit)} commission credited to the reseller.`); } saveState(); refreshShell(); renderOrders(); }
function renderProfile() { const content = document.querySelector("#profile-content"); if (!isLoggedIn()) { content.innerHTML = `<div class="empty-state">Log in to view your profile.</div>`; return; }   const userOrders = state.orders.filter(order => order.resellerEmail === state.account.email); const earnings = userOrders.reduce((sum, order) => sum + (order.status === "Delivered" ? order.profit : 0), 0); content.innerHTML = `<article class="profile-card wallet-card"><span class="eyebrow">${state.account.role}</span><h3>${state.account.name}</h3><p>Available demo balance</p><div class="balance">${money(currentWallet())}</div><div>${userOrders.length} total orders · ${money(earnings)} earned</div></article><article class="profile-card"><h3>Account details</h3><p><strong>Email:</strong> ${state.account.email}</p><p><strong>Role:</strong> ${state.account.role}</p><p><strong>Commission rule:</strong> credited only after delivered status and a tracking link.</p></article>`; }
function openProductModal() { if (currentRole() !== "wholesaler") return showToast("Log in as a wholesaler to upload products."); document.querySelector("#product-modal").classList.remove("hidden"); }
function createProduct(event) { event.preventDefault(); const product = { id: Date.now(), name: document.querySelector("#product-name").value.trim(), category: document.querySelector("#product-category").value.trim(), price: Number(document.querySelector("#product-price").value), stock: Number(document.querySelector("#product-stock").value), seller: state.account.name, ownerEmail: state.account.email, image: document.querySelector("#product-image").value.trim() || defaultProducts[0].image, description: document.querySelector("#product-description").value.trim() }; if (!product.name || product.price <= 0 || product.stock <= 0) return showToast("Complete the product details first."); state.products.unshift(product); saveState(); closeModals(); document.querySelector("#product-form").reset(); showToast("Product published to the catalogue."); navigate("orders"); }

document.addEventListener("click", event => { const routeButton = event.target.closest("[data-route]"); if (routeButton) navigate(routeButton.dataset.route); const authButton = event.target.closest("[data-open-auth]"); if (authButton) openAuth(authButton.dataset.openAuth, authButton.dataset.role || "reseller"); if (event.target.closest("[data-close-modal]")) closeModals(); const roleButton = event.target.closest("[data-role-choice]"); if (roleButton) { selectedRole = roleButton.dataset.roleChoice; updateAuthModal(); } const modeButton = event.target.closest("[data-auth-mode]"); if (modeButton) { authMode = modeButton.dataset.authMode; updateAuthModal(); } });
document.querySelector("#auth-form").addEventListener("submit", handleAuth);
document.querySelector("#product-form").addEventListener("submit", createProduct);
document.querySelector("#add-product-button").addEventListener("click", openProductModal);
document.querySelector("#search-input").addEventListener("input", renderCatalog);
refreshShell();
